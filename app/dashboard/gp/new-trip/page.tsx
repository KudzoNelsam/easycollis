...
import { CITIES_DEPART, CITIES_DESTINATION, type Trip } from "@/lib/data"
...
export default function NewTripPage() {
  ...
  useEffect(() => {
    ...
    if (editId && user) {
      const savedTrips = JSON.parse(localStorage.getItem(`easycollis_trips_${user.id}`) || "[]")
      const tripToEdit = savedTrips.find((t: Trip) => t.id === editId)
      if (tripToEdit) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const existingTrips = JSON.parse(localStorage.getItem(`easycollis_trips_${user.id}`) || "[]")
    ...
    if (isEditing) {
      ...
      // Mode création
      const newTrip: Trip & { cityDepart: string } = {
        id: `trip-${Date.now()}`,
        gpId: user.id,
        gpName: user.name,
        cityDepart,
      ...
      localStorage.setItem(`easycollis_trips_${user.id}`, JSON.stringify(existingTrips))
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 py-8">
        <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
          ...
          <Card>
            <CardHeader>
              <CardTitle>{isEditing ? "Modifier le voyage" : "Créer un nouveau voyage"}</CardTitle>
              <CardDescription>
                {isEditing
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select value={cityDepart} onValueChange={setCityDepart} required>
                ...
                <div className="space-y-2">
                  <Label htmlFor="price">Prix par kg (FCFA)</Label>
                  <Input
                    id="price"
                    type="number"
                    placeholder="5000"
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    id="description"
                    placeholder="Décrivez votre voyage, les conditions de transport..."
                    value={description}
                ...
                <div className="flex gap-4">
                  <Button type="button" variant="outline" onClick={() => router.push("/dashboard/gp")}>
                    Annuler
                  </Button>
                  <Button
                    type="submit"
