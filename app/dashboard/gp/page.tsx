...
import { useAuth, type GPProfile } from "@/lib/auth-context"
                        ...
                        <div key={trip.id} className="flex items-center justify-between p-4 rounded-lg border">
                          ...
                          <div className="flex items-center gap-2">
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => router.push(`/dashboard/gp/new-trip?edit=${trip.id}`)}
                            >
                ...
                <CardContent className="space-y-3">
                  ...
                  {gpUser.departureDate && (
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span>{new Date(gpUser.departureDate).toLocaleDateString("fr-FR")}</span>
                    </div>
                  )}
                  {gpUser.availableKg && (
                    <div className="flex items-center gap-2 text-sm">
                          ...
                          <Link key={conv.id} href="/messages" className="block">
                            <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted transition-colors">
                              <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center">
                                <span className="text-sm font-semibold text-primary">{otherParticipant?.name[0]}</span>
                              </div>
