...
function SearchContent() {
  ...
  useEffect(() => {
    ...
    listGPs()
      .then((data) => {
        if (!mounted) return;
        // listGPs may return either an array (legacy) or a paginated object { items, total }
        setGps(Array.isArray(data) ? data : data.items || []);
      })
      .catch((err) => {
  ...
  const filteredGPs = useMemo(() => {
    ...
    if (destination) {
      results = results.filter((gp) =>
        gp.destination.toLowerCase().includes(destination.toLowerCase())
      );
    }
    if (date) {
    ...
    if (city) {
      results = results.filter((gp) =>
        gp.city.toLowerCase().includes(city.toLowerCase())
      );
    }
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-6">
            <p className="text-muted-foreground flex items-center gap-3">
              <span>
                {filteredGPs.length} GP{filteredGPs.length > 1 ? "s" : ""}{" "}
                trouvé{filteredGPs.length > 1 ? "s" : ""}
              </span>
              {justUpdated && (
                <span className="inline-block rounded-full bg-green-50 text-green-700 text-xs px-2 py-0.5">
          ...
          {filteredGPs.length > 0 ? (
            ...
            <div className="text-center py-12">
              <p className="text-muted-foreground">
                Aucun GP trouvé pour ces critères.
              </p>
